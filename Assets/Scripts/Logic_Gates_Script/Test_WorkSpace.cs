...
public class Test_WorkSpace : MonoBehaviour
{
    ...
    {
        SearchAndPrintCable(ConnectorType.In,Or8Way_Gate.TestIN(),CableTypes.c16);
    }
    void Start()
    {
        ...
        else if (Name == LogicGates_Type.Mux16)
        {
            Mux16_Gate_tests();
        }
        else 
    ...
    void Update()
    {
        ...
        for (int i=0; i<cableManagaer16TruthTables.Count ;i++)
        {
            ...
            if (cableManagaer16TruthTables[i] != null  && cableManagaer16TruthTables[i].cableManagers.GetCable16Saved() != null)
            {
                ...
                else if (Name == LogicGates_Type.Mux16)
                {
                    if(cableManagerName == ConnectorType.Out)
                    {
                        if (!CheckTruth16Table(actualTruthTable, Mux16_Gate.TestOUT()))
                        {
                            challengeUI_fail.SetActive(true);
                            isSolves16=false;
                            break;
        ...
        //             if(cableManagerName == ConnectorType.Out)
        //             {
        //                 if (!CheckTruth16Table(actualTruthTable, Or8Way_Gate.TestOUT()))
        //                 {
        //                     isSolves16=false;
        //                     break;
        //                 }
        ...
        for (int i=0; i<cableManagaerTruthTables.Count ;i++)
        {
            if (cableManagaerTruthTables[i] != null && cableManagaerTruthTables[i].cableManager.getConnectedCable() != null )
            {
                ...
                else if (Name == LogicGates_Type.Dmux)
                {
                    ...
                    if(cableManagerName == ConnectorType.InA)
                    {
                        if (!CheckTruthTable(actualTruthTable, Dmux_Gate.TestA()))
                        {
                            challengeUI_fail.SetActive(true);
                            isSolves=false;
                            break;
                    ...
                    {
                        if (!CheckTruthTable(actualTruthTable, Dmux_Gate.TestB()))
                        {
                            challengeUI_fail.SetActive(true);
                            isSolves=false;
                            break;
                ...
                else if (Name == LogicGates_Type.Mux)
                {
                    if(cableManagerName == ConnectorType.Out)
                    {
                        if (!CheckTruthTable(actualTruthTable, Mux_Gate.TestOut()[0].truthTable))
                        {
                            isSolves=false;
                            challengeUI_fail.SetActive(true);
                            break;
                ...
                else if(Name == LogicGates_Type.Or4Way )
                {
                    if(cableManagerName == ConnectorType.Out)
                    {
                        if (!CheckTruthTable(actualTruthTable, Or8Way_Gate.TestOUT()))
                        {
                            challengeUI_fail.SetActive(true);
                            isSolves=false;
                            break;
                        ...
                        count++;
                        isSolves=true;
                        // Debug.LogError($"i find {cableManagerName}  cableTruthTables.");
                    }
                }
                else
                {
                    Debug.LogError($"i dont find {Name}  work space.");
                } 
                // Debug.Log($"i find {cableManagerName}  cableTruthTables.");
            }
        }
        ...
        if(isSolves16 && count16 == cableManagaer16TruthTables.Count )
        {
            ...
            if (cableManagaerTruthTables.Count == 0)
            {
                ...
                Debug.Log("All tests passed for 16-bit gate!");
                // door.OpenDoor();
            }
        }
        if (isSolves && count == cableManagaerTruthTables.Count)
        {
            Debug.LogError($"i find   cableTruthTables.");
            isAllCorrect=true;
            if (cableManagaer16TruthTables.Count == 0 )
            {
                // Debug.Log("All tests passed for 1-bit gate!");
                ...
                ComponentBarManager.SolvedComponents[Name] = true;
    ...
    private bool CheckTruth16Table(List<Cable16bitTruthTable> truthTable,List<Cable16bitTruthTable> actualTruthTable)
    {
        ...
        {   
            for(int i=0;i<actualTruthTable.Count;i++)
            {
                ...
                {
                    Debug.LogError($"i does not correct {i}  work space.");
                    return false;
                }
                for (int j=0;j< actualTruthTable[i].truthTable.Count ;j++)
                {
                    ...
                    {
                        Debug.LogError($"i does not correct {i}  work space.");
                        Debug.Log("Input  actualTruthTable: ");
                       
                        Debug.Log("  "+i +" "+ string.Join(", ", actualTruthTable[i].truthTable));
                        
                        Debug.Log("Input  truthTable: ");
            ...
            for(int i=0;i<actualTruthTable.Count;i++)
            {
                if(actualTruthTable[i] != truthTable[i])
                {
                    Debug.LogError($"i does not correct {i}  work space.");
                        ...
                        Debug.Log("  "+i +" "+ string.Join(", ", actualTruthTable[i]));
                        
                        Debug.Log("Input  truthTable: ");
                        
                        Debug.Log("  "+i +" "+ string.Join(", ", truthTable[i]));
                    return false;
                }  
            }
            return true;  
