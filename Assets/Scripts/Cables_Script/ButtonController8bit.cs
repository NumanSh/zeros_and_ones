...
public class ButtonController8bit : MonoBehaviour
{
    ...
    private List<Cable> spawnedCables = new List<Cable>(new Cable[8]);
    ...
    public void OnBitSelected(int bitIndex)
    {
        ...
        if( bitIndex >=0 && bitIndex <=7 && currentState == GameState.SelectingBit)
        {
            Debug.Log($"Selected bit: {bitIndex}");
